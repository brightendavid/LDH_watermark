...
from model_ldh.modules import BasicBlock, Bottleneck
...
class HRNet(nn.Module):
    def __init__(self, c=48, nof_joints=1, bn_momentum=0.1):
        ...
        # self.convT1 = torch.nn.ConvTranspose2d(nof_joints, nof_joints, kernel_size=(2, 2),stride=2)
        # self.convT2 = torch.nn.ConvTranspose2d(nof_joints, nof_joints, kernel_size=(2, 2), stride=2)

    def forward(self, x):
        # stage 1
        x = self.conv1(x)  # 两个卷积造成分辨率变为原本的1/4
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        x = self.bn2(x)
...
if __name__ == '__main__':
    """
    经过hr net之后的图像分辨率变为原来的1/4*1/4
    """
    model = HRNet(36, 1, 0.1)
