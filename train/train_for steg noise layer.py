...
name = now_day + 'watermark_noiselayer shiyan'
...
def noise_layer_stega(contain, OUT_SECRET, epoch, is_test=False):
    # 模拟stegastamp的noise layer结构
    if is_test:
        batch_size = 1
    else:
...
def train(Enet, Hnet, Rnet, optimizer, dataParser, epoch):
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(True):
            ...
            contain, OUT_SECRET = noise_layer_stega(contain, OUT_SECRET, epoch)
    ...
    for batch_index, input_data in enumerate(dataParser):
        if batch_index % 8 == 0:
            ...
            with torch.set_grad_enabled(False):
                ...
                contain, _ = noise_layer_stega(contain, secrets_in, epoch, is_test=True)
