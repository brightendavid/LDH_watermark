...
from model_ldh import Revealnet, Hide_net_deep, Encode_net

now_day = datetime.datetime.now().date()
now_day = str(now_day)[-5:]
# 命名格式为日期（月+日） +  train重要信息  根据日期命名runs文件
name = now_day + 'watermark_TObelast'
...
parser.add_argument('--resume', default=[
    "/home/liu/sjw/save_model/watermark_base/BEST_Encode_07-25watermark_TObelast_checkpoint17.pth",
    "/home/liu/sjw/save_model/watermark_base/BEST_Hide_07-25watermark_TObelast_checkpoint17.pth",
    "/home/liu/sjw/save_model/watermark_base/BEST_Rev_07-25watermark_TObelast_checkpoint17.pth",
    # "/home/liu/sjw/save_model/07-19watermark_nosie_genel/BEST_Encode_07-19watermark_nosie_genel_checkpoint12.pth",
    # "/home/liu/sjw/save_model/07-19watermark_nosie_genel/BEST_Hide_07-19watermark_nosie_genel_checkpoint12.pth",
    # "/home/liu/sjw/save_model/07-19watermark_nosie_genel/BEST_Rev_07-19watermark_nosie_genel_checkpoint12.pth"
    # "/home/liu/sjw/save_model/watermark_base/BEST_Encode_07-23watermark_deephide_checkpoint1.pth",
    # "/home/liu/sjw/save_model/watermark_base/BEST_Hide_07-23watermark_deephide_checkpoint1.pth",
    # "/home/liu/sjw/save_model/watermark_base/BEST_Rev_07-23watermark_deephide_checkpoint1.pth"
...
def noise_layer(contain, OUT_SECRET, epoch=0, is_test=False):
    pretain = False
    if pretain == True:
        if is_test:
            batch_size = 1
        ...
        if is_test == True or random.randint(0, 9) % 4 != 1:
            ...
            elif epoch < 30 and random.randint(0, 9) % 4 == 1:
...
def noise_layer3(contain, OUT_SECRET, epoch):
    t1 = random.randint(1, 5) / 10  # t从0.1-0.5都没有问题，可能会模糊
    t2 = random.randint(1, 5) / 10
    contain = warped(contain, t1, t2)
    OUT_SECRET = warped(OUT_SECRET, t1, t2)
...
def noise_layer5(contain, OUT_SECRET, epoch=0, is_test=False):
    ...
    if is_test == True or random.randint(0, 9) % 4 != 1:
        ...
        if random.randint(0, 2) % 3 == 1:
            ...
            a = epoch / 100
            theta = np.min([a, 1.0])
            brighten_layer = get_rnd_brightness(0.2, 0.2, batch_size).cuda()
...
def main():
    ...
    Hnet = Hide_net_deep.HideNet(input_nc=3+1,
...
def train(Enet, Hnet, Rnet, optimizer, dataParser, epoch):
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(True):
            ...
            contain, OUT_SECRET = noise_layer5(contain, OUT_SECRET, epoch)
            ...
            Loss_all = loss_o_sec + loss_lpips_mean *0.5 + 0.1 * loss_res + (
                    1 - ssim) * 0.3  # + color_loss * 2  # 最终的loss
...
def val(Enet, Hnet, Rnet, dataParser, epoch):
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(False):
            ...
            Loss_all = loss_o_sec + loss_lpips_mean * 0.5 + 0.1 * loss_res + (
                    1 - ssim) * 0.3  # + 10 * color_loss  # 最终的loss
    ...
    for batch_index, input_data in enumerate(dataParser):
        if batch_index % 11 == 0:
            ...
            with torch.set_grad_enabled(False):
                ...
                contain, _ = noise_layer5(contain, secrets_in, epoch, is_test=True)
