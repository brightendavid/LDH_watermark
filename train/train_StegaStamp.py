...
def main():
    ...
    Discriminator = stega_net.StegaStampDiscriminator().to("cuda")  # discriminator
    ...
    # optimizer_d = optim.RMSprop(d_vars, lr=0.00001)
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        ...
        train_avg = train(Hnet=Encoder, Rnet=Decoder, Dnet=Discriminator, optimizer=optimizer_g,
                          dataParser=trainDataLoader, epoch=epoch)
        val_avg = val(Hnet=Encoder, Rnet=Decoder, Dnet=Discriminator, dataParser=valDataLoader, epoch=epoch)
        ...
        output_name2 = 'Dis' + output_name_file_name % (epoch)
        ...
        if is_bestpth(val_avg['loss_avg']):
            ...
            save_model_name_stage2 = os.path.join(args.model_save_dir, 'Dis' + output_name2)
            torch.save({'epoch': epoch, 'state_dict': Encoder.state_dict(), 'optimizer': optimizer_g.state_dict()},
                       save_model_name_stage0)
            torch.save({'epoch': epoch, 'state_dict': Decoder.state_dict(), 'optimizer': optimizer_g.state_dict()},
                       save_model_name_stage1)
            # torch.save(
            #     {'epoch': epoch, 'state_dict': Discriminator.state_dict(), 'optimizer': optimizer_d.state_dict()},
            #     save_model_name_stage2)
    print('训练已完成!')


def train(Hnet, Rnet, Dnet, optimizer, dataParser, epoch):
    # 读取数据的迭代器
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(True):
            ...
            Loss_all = loss_o_sec * 2 + loss_lpips_mean * 0.7 + loss_res * 0.1 + ent_losss * 0.1 + (
                    1 - ssim) * 0.1  # 最终的loss
...
def val(Hnet, Rnet, Dnet, dataParser, epoch):
    # val 模块不加入扰动
    train_epoch = len(dataParser)
    batch_time = Averagvalue()
    data_time = Averagvalue()
...
def test(Hnet, Rnet, dataParser, epoch):  # 测试集
    ...
    for batch_index, input_data in enumerate(dataParser):
        ...
        with torch.set_grad_enabled(False):
            ...
            # str(outputs[batch_idx].item()), epoch)
